...
class IssueStatusUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields=[
            "title","description",
            "apartment","reported_by","resolved_by",
            "status","resolved_on"
        ]
    def update(self, instance:Issue, validated_data:dict):
        if(validated_data.get("status")==Issue.IssueStatus.RESOLVED and 
           instance.resolved_on!=Issue.IssueStatus.RESOLVED):
