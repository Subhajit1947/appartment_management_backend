...
class PostDetailsAPIView(generics.RetrieveAPIView):
    ...
    def get_object(self)->Post:
        ...
        filter_kwags={self.lookup_field:self.kwargs[self.lookup_field]}
        post=generics.get_object_or_404(queryset,**filter_kwags)
    ...
    def get_object(self):
        ...
        if post.author!=self.request.user:
            raise PermissionDenied(
                "yo do not have permission to edit this post"
            )
        return post
...
class BookmarkPostAPIView(APIView):
    def patch(self,request,slug):
        ...
        return Response(
            {"message":"Post bookmarked"},status=status.HTTP_200_OK
        )
...
class UnBookmarkPostAPIView(APIView):
    def patch(self,request,slug):
        ...
        if user not in post.bookmarked_by.all():
            return Response(
                {
                    "message":"you can not remove a bookmarkthat did not exist in the first place" 
                },
                status=status.HTTP_400_BAD_REQUEST
            )
        post.bookmarked_by.remove(user)
        return Response(
            {"message":"Post bookmarked removed"},status=status.HTTP_200_OK
        )
...
class UpvotesPostAPIView(APIView):
...
class DownvotesPostAPIView(APIView):
    def patch(self,request,post_id):
        post=get_object_or_404(Post,id=post_id)
        serializer=DownvotePostSerializer(post,data=request.data,context={"request":request})
        if serializer.is_valid():
            serializer.save()
            return Response(
