...
class Post(TimeStampedModel):
    ...
    upvotes=models.PositiveIntegerField(
        default=0,verbose_name=_("Upvotes")
    )
    ...
    def get_popular_tags(cls,limit=5):
        return cls.tags.annotate(post_count=Count("taggit_taggeditem_items"))\
            .order_by("-post_count")[:limit]
    
    def save(self,*args,**kwargs):
        if not (self.author.is_superuser or 
            self.author.is_staff or 
            self.author.profile.occupation==Profile.Occupation.TENANT
        ):
            raise ValueError(
                "only tenants,superuser or stuff members can create posts"
            )
...
class Reply(TimeStampedModel):
    post=models.ForeignKey(
        Post,on_delete=models.CASCADE,verbose_name=_("Post"),related_name="replies"
    )
    author=models.ForeignKey(
        User,verbose_name=_("Author"),on_delete=models.CASCADE,
