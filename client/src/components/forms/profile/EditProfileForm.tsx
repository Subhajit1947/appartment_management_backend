...
import { Contact2Icon, Map, MapPinnedIcon, UserCheck } from 'lucide-react'
...
export default function EditProfileForm() {
    ...
    const {register,handleSubmit,reset,setValue,control,formState:{errors}}=useForm<TProfileSchema>()
    ...
    const uploadFileHandler=async(e:React.ChangeEvent<HTMLInputElement>)=>{
        ...
        try {
            ...
            const {data}=await axios.patch("/api/v1/profile/user/avatar/",formData,config)
            setAvatar(data)
            setUploading(false)
        } catch (error) {
    ...
    <main>
        <form noValidate onSubmit={handleSubmit(onSubmit)} className='flex w-full max-w-md flex-col gap-4'>
            <FormFieldComponent
                ...
                startIcon={<UserCheck className='dark:text-babyPowder size-8'/>}
            />
            <FormFieldComponent
                label='First Name'
                name='first_name'
